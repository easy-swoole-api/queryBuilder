...
abstract class Builder
{
            ...
            if (is_null($val)) {
                ...
                switch (strtolower($val[0])) {
                    ...
                    case 'dec':
                        ...
                        break;
                    case 'exp':
                        throw new BuilderException('not support data:[' . $val[0] . ']');
                }
            } elseif (is_scalar($val)) {
                // 过滤非标量数据
                if (0 === strpos((string)$val, '?') && $this->query->isBind(substr($val, 1))) {
                    $result[$item] = $val;
                } else {
                    $key = str_replace('.', '_', $key);
                    $this->query->bind('data__' . $key, $val, isset($bind[$key]) ? $bind[$key] : ParamEnum::PARAM_STR);
    ...
    protected function parseWhereItem($field, $val, $rule = '', $options = [], $binds = [], $bindName = null)
    {
        ...
        if (!in_array($exp, $this->exp)) {
            ...
            if (isset($this->exp[$exp])) {
                ...
                throw new BuilderException('where express error:' . $exp);
            }
        }
        ...
        $bindType = isset($binds[$field]) ? $binds[$field] : ParamEnum::PARAM_STR;
        ...
        if (in_array($exp, ['=', '<>', '>', '>=', '<', '<='])) {
            ...
            if (is_array($value)) {
                foreach ($value as $item) {
                    $array[] = $key . ' ' . $exp . ' ' . $this->parseValue($item);
                }
    ...
    public function insertAll($dataSet, $options = [], $replace = false)
    {
        ...
        foreach ($insertFields as $field) {
            $fields[] = $this->parseKey($field, $options);
        }
